...
export class ResponseService {
	...
	public async findById(id: string) {
		try {
			...
			if (!response) throw httpException("Response not found.", HttpStatus.NOT_FOUND);
	...
	private calculateQuickInterval(interval: string): { startDate: Date; endDate: Date } {
		const endDate = new Date();
		const startDate = new Date(endDate);
	  
		const match = interval.match(/^(\d+)(min|hr|d|w|m)$/);
		if (!match) {
		  throw httpException("Invalid quickInterval format. Use: '1hr', '2d', '30min', '1w', '1m'", 400);
		}
